...
(function(){
	...
	Question.prototype.showScore = function(scr) {

		console.log("Your current score is: " + scr);
		console.log("---------------------");
...
(function() {
    ...
    function nextQuestion() {
        ...
        if(answer !== 'exit') {
            questions[n].checkAnswer(parseInt(answer), keepScore); // check is only run if prompt is not 'exit' the prompt is parsed and score() is passed as a callback
            
            nextQuestion();
        }
    }
    
    nextQuestion(); // function immediately called game logic is performed and questions presented
